...
# A friendly little help message.
if len(sys.argv) < 5:
    print("Hello! This is a script that computes enrichment statistics for an in-vitro selection experiment.")
    print("The 4 required arguments are the EasyDIVER count.aa outputs. Notably, the files for the input, neg, out experiments and the file name for the results of this script.")
    print("Arguments:")
    print("  in_file       : File name for the input file (.txt)")
    print("  neg_file       : File name for negative control file (.txt)")
    print("  postsel_file   : File name for post-selection file (.txt)")
    print("  res_file       : File name for output file (.txt)")
    # print("  naming_ref     : Reference file name") Name your sequences based on rank
    # print("  alph           : Value for alph") Reduce amino acid pool
    sys.exit(1)

print(sys.argv[1])
print(sys.argv[2])

in_file  = sys.argv[1] # '3-5R5-Fus-in_S3_counts_aa_L.txt'
...
res_file      = sys.argv[4] # '5R5-ATP_counts_aa_L_enr.txt'
...
files = [in_file, neg_file, postsel_file]
...
for in_file in files:
    ...
    with open(in_file) as in_data:
        ...
        for line in in_data:
            ...
            # deg = int(line.split()[3]) TODO: Caused problems... fix?
            ...
            if len(seq) > max_len:
                max_len = len(seq)
        seqfit_list = [[seqs[i], ( abunds[i],fracs[i], ranks[i])] for i in range(len(seqs))] #degs[i] as well
...
print(str('number of unique sequences fus / neg / post = ') + str(uniques[0]) + str(" / ") + str(uniques[1]) + str(" / ") + str(uniques[2]), file=out)
print(str('total number of molecules fus / neg / post = ') + str(totals[0]) + str(" / ") + str(totals[1]) + str(" / ") + str(totals[2]), file=out)
print(str('rank').ljust(10), file=out)
print(str('seq').ljust(max_len), file=out)

print(str('a_cDNA').ljust(10), str('f_cDNA').ljust(20), file=out)

print(str('a_cDNA_l').ljust(10), str('f_cDNA_l').ljust(20), file=out)
print(str('a_cDNA_u').ljust(10), str('f_cDNA_u').ljust(20), file=out)

print(str(str('a_') + str(postsel_file.split("_")[0]).split("-")[-1]).ljust(10), file=out)
print(str(str('f_') + str(postsel_file.split("_")[0]).split("-")[-1]).ljust(20), file=out)

print(str(str('a_') + str(neg_file.split("_")[0]).split("-")[-1]).ljust(10), file=out)
print(str(str('f_') + str(neg_file.split("_")[0]).split("-")[-1]).ljust(20), file=out)

print(str(str('a_') + str(neg_file.split("_")[0]).split("-")[-1] + str("_l")).ljust(10), file=out)
print(str(str('f_') + str(neg_file.split("_")[0]).split("-")[-1] + str("_l")).ljust(20), file=out)

print(str(str('a_') + str(neg_file.split("_")[0]).split("-")[-1] + str("_u")).ljust(10), file=out)
print(str(str('f_') + str(neg_file.split("_")[0]).split("-")[-1] + str("_u")).ljust(20), file=out)

print(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1]).ljust(20), file=out)
print(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1] + str("_l")).ljust(20), file=out)
print(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1] + str("_u")).ljust(20), file=out)

print(str(str('e_neg')).ljust(20), file=out)
print(str(str('e_neg') + str("_l")).ljust(20), file=out)
print(str(str('e_neg') + str("_u")).ljust(20), file=out)

print(str(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1]) + ("/") + str(str('e_neg'))).ljust(20), file=out)

print(str(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1]) + ("/") + str(str('e_neg_u'))).ljust(20), file=out)
print(str(str(str('e_') + str(postsel_file.split("_")[0]).split("-")[-1]) + ("/") + str(str('e_neg_l'))).ljust(20), file=out)

print(str(str('ill')).ljust(10), file=out)
print(str(str('n_ill')).ljust(10), file=out)
print(str(str('from_red')).ljust(20), file=out)

print(str(str('deg')).ljust(10), file=out)
...
for seq in all_dict[2]:
    ...
    # deg_post = all_dict[2][seq][3] TODO: deg issues
    ...
    # if alph == '5aa':  We assume standard AA count
    #
    # 	#illegal:
    ...
    if float(f_post) > 0.0000711526:
        ...
        # print(str(str(sel) + str("-") + str("V") + str(rank_post)).ljust(10), file=out)
        ...
        print(str(c_fus).ljust(10), file=out)
        print(str('%.10f' % float(f_fus)).ljust(20), file=out)

        ##### abd and freq fusion LOWER and UPPER

        print(str(c_fus_l).ljust(10), file=out)
        if type(f_fus_l) == float:
            print(str('%.10f' % float(f_fus_l)).ljust(20), file=out)
        else:
            print(str(f_fus_l).ljust(20), file=out)

        print(str(c_fus_u).ljust(10), file=out)
        if type(f_fus_u) == float:
            print(str('%.10f' % float(f_fus_u)).ljust(20), file=out)
        else:
            print(str(f_fus_u).ljust(20), file=out)

        ##### abd and freq post

        print(str(c_post).ljust(10), file=out)
        print(str('%.10f' % float(f_post)).ljust(20), file=out)

        ##### abd and freq neg

        print(str(c_neg).ljust(10), file=out)
        print(str('%.10f' % float(f_neg)).ljust(20), file=out)

        ########################################################################### new
        ##### abd and freq neg LOWER and UPPER

        print(str(c_neg_l).ljust(10), file=out)
        if type(f_neg_l) == float:
            print(str('%.10f' % float(f_neg_l)).ljust(20), file=out)
        else:
            print(str(f_neg_l).ljust(20), file=out)

        print(str(c_neg_u).ljust(10), file=out)
        if type(f_neg_u) == float:
            print(str('%.10f' % float(f_neg_u)).ljust(20), file=out)
        else:
            print(str(f_neg_u).ljust(20), file=out)
        ###########################################################################

        ##### enr post

        print(str('%.10f' % float(enr_post)).ljust(20), file=out)

        ##### enr post LOWER and UPPER

        if type(enr_post_u) == float:
            print(str('%.10f' % float(enr_post_l)).ljust(20), file=out)
            print(str('%.10f' % float(enr_post_u)).ljust(20), file=out)
        else:
            print(str(enr_post_l).ljust(20), file=out)
            print(str(enr_post_u).ljust(20), file=out)

        ##### enr neg

        print(str('%.10f' % float(enr_neg)).ljust(20), file=out)

        ##### enr neg LOWER and UPPER ####################################### just modified

        if type(enr_neg_u) == float:
            print(str('%.10f' % float(enr_neg_l)).ljust(20), file=out)
            print(str('%.10f' % float(enr_neg_u)).ljust(20), file=out)
        else:
            print(str(enr_neg_l).ljust(20), file=out)
            print(str(enr_neg_u).ljust(20), file=out)
        ###########################################################################

        ##### rel enr

        if float(enr_neg) > 0:
            print(str('%.10f' % float(enr_post / enr_neg)).ljust(20), file=out)
        else:
            print(str("-").ljust(20), file=out)
        ...
        if type(enr_neg_u) == float:
            print(str('%.10f' % float(enr_post / enr_neg_u)).ljust(20), file=out)
        else:
            print(str("-").ljust(20), file=out)
        if type(enr_neg_u) == float:
            print(str('%.10f' % float(enr_post / enr_neg_l)).ljust(20), file=out)
        else:
            print(str("-").ljust(20), file=out)
        ...
        print(str(seq_ill).ljust(10), file=out)
        print(str(seq_n_ill).ljust(10), file=out)
        print(str(seq_from_red).ljust(10), file=out)

        # print(str(deg_post).ljust(10), file=out)
    ...
    out2 = open(res_file, 'w')
    count_line = 0
    for line in out:
        count_line += 1
        if count_line == 1:
            print(str(line.split("\n")[0] + str(" | number of unique sequences post = ") + str(new_unique)), file=out2)
        if count_line == 2:
            print(str(line.split("\n")[0] + str(" | total number of molecules = ") + str(new_total)), file=out2)
