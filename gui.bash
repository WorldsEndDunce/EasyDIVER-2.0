...
function diveAndSail {
  ...
  whiptail --title "Finished" --msgbox "Continuing on to calculate enrichments." 10 40
      ...
      if [ ! -n "$(find "$counts_dir" -name "$in_format" -print -quit)" ]; then
          ...
          for ((i = 1; i < max_round; i++)); do
              ...
              if [ ! -n "$(find "$counts_dir" -name "$neg_format" -print -quit)" ]; then
                ...
                python3 ./scripts_enrichments/modified_counts.py -out "$counts_dir/$i-out"*"_$counts_type.txt" -neg "$counts_dir/$(($i + 1))-neg"*"_$counts_type.txt" -res "$outdir/modified_counts/$i-res.txt"
              fi
