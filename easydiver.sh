...
v2.0 Author: Allison Tee (ateecup@stanford.edu)
EOF
...
#============================TODO: Find some way to compress the neg control section with the script for the normal data analysis=========================================
#if [-z $negopt ]; then
#  # move to working directory
#  # make output directories
#  cd $outdir
#  mkdir neg 2>/dev/null
#  outdir="$outdir/neg"
#  cd $outdir
#  mkdir counts individual.lanes fastqs fastas histos 2>/dev/null
#
#  # loop through reads & process them
#  for R1 in $neg_fastqs/*R1*
#  do
#
#    # Define some general use variables
#    basename=$(basename ${R1})
...
#
#  ########## CREATE LOG FILE FOR DNA AND PEPTIDE SEQUENCES ##########
#  # TODO: Fix this part, for it is buggy
#      cd ..
#
#      echo ""  >> $outdir/log.txt
#      echo "sample" "fastq_R1" "fastq_R2" "unique_nt" "total_nt" "recovered_nt(%)" "unique_aa" "total_aa" "recovered_aa(%)"| column -t > $outdir/log_temp1.txt
